...
import { CampoBuscaComponent, PaginacaoComponent, TagStatusComponent } from '../../../../compartilhado/componentes';
// TagStatusComponent removido - usando badges customizados

// Componente de Upload
import { ModalUploadComponent } from '../../componentes/modal-upload/modal-upload.component';

// Models e Services
import { RespostaPaginada, StatusDocumento, TipoDocumento } from '../../../../compartilhado/modelos';
...
export class ListaDocumentosComponent implements OnInit {
  ...
  private carregarParceiros(): void {
    this.parceiroService.getParceiros(1, 100).subscribe({
      next: (resposta) => {
        const opcoes = resposta.data.map((parceiro: any) => ({
  ...
  getTipoLabel(tipo: string): string {
    return TIPOS_DOCUMENTO_LABELS[tipo as keyof typeof TIPOS_DOCUMENTO_LABELS] || tipo;
  }
  ...
  getStatusParaTag(status: string): string {
    switch (status) {
      case 'aprovado':
        return 'ativo';
      case 'rejeitado':
  ...
  getActiveFiltersCount(): number {
    let count = 0;
    if (this.pesquisaDocumentos()) count++;
    if (this.filtroStatus) count++;
    if (this.filtroTipo) count++;
    if (this.filtroParceiro) count++;
    if (this.filtroVencimento) count++;
    return count;
  }
}
